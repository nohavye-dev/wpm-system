...
function addItemArgs(
  first: string | AddOptions,
  rest: string[],
): { items: string[]; before: boolean } {
...
export class PromptTask {
  ...
  public clone(): PromptTask {
    return new PromptTask(this.name, [...this.instructions], [...this.constraints])
  }
  ...
  public toString(level = 2): string {
    ...
    const lines: string[] = [`${heading}# ${this.name}`, ""]
...
export class PromptContext {
  ...
  public toString(): string {
    ...
    if (this.purpose.length > 0) {
      lines.push("## Purpose", "", ...this.purpose.map((item) => `  - ${item}`), "")
    }
