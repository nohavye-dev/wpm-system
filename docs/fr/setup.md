...
- Toutes les clés documentées de `wpm.config.json` sont éditables à la main : `db_path` (choix du dossier/nom de la base), `confidence_threshold`, `idle_nudge` (relance opt-in en session inactive), `embedding`, `domain` (tuning avancé du scoring). `wpm enable` les préserve et ne remplit que les clés absentes.
