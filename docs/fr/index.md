...
- [`commands.md`](commands.md) — workflows `wpm-doc`, `wpm-code`, `wpm-review`, `wpm-bootstrap` et `wpm-patterns` (prompts MCP) : ingestion contrôlée d'un document, cartographie du code, revue de la santé de la mémoire, bootstrap initial du projet, et analyse de patterns.
...
Les workflows `wpm-doc`, `wpm-code`, `wpm-review`, `wpm-bootstrap` et
`wpm-patterns` sont des **prompts MCP** exposés par le serveur (déclarés
dans `wpm-mcp-server/src/wpm_mcp_server/server.py`). Dans opencode ils
apparaissent comme commandes slash (ex. `/wpm:wpm-doc:mcp`) ; dans tout
